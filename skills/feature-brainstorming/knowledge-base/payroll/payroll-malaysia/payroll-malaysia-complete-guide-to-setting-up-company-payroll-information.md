---
title: "Payroll Malaysia: Complete Guide to Setting Up Company Payroll Information"
category: "Payroll"
subcategory: "Payroll - Malaysia"
source_url: "https://support.briohr.com/knowledge/how-to-set-up-company-payroll-general-settings-in-malaysia"
date: "August 19, 2026"
---

# Payroll Malaysia: Complete Guide to Setting Up Company Payroll Information

*Learn how to properly set up your company’s payroll general settings in brioHR, including registration details, bank information, salary proration, statutory setup, payslip settings, and more. Includes step-by-step instructions and FAQs.*

**Audience & Scope**


Audience: HR Admin


Module: Payroll


Country: Malaysia


Pre-requisites: User group settings - Access to HR Lounge > Payroll > Module Settings


### Introduction


Setting up your company payroll information is one of the most important steps when creating a new payroll center in brioHR. This step up includes essential details such as:


- Company registration and statutory information
- Bank account details
- Salary proration and calculation rules
- Payslip visibility settings


**IMPORTANT:**

Incomplete Payroll General Settings can cause issues when processing monthly payroll and accessing payroll documents.


This guide walks you through each section step-by-step to ensure your Malaysia payroll center is correctly configured


---


### How to Access Payroll General Settings


#### Step 1 - Log in to your BrioHR Account


#### Step 2 - Navigate to Payroll


1. Click **HR Lounge.**


2. Select **Payroll.**


You will be directed to the Payroll Dashboard.


![image](https://support.briohr.com/hs-fs/hubfs/image-png-Dec-09-2025-04-19-15-5427-AM.png?width=670&height=300&name=image-png-Dec-09-2025-04-19-15-5427-AM.png)


#### Step 3 - Go to Settings


1. Click **Settings.**


2. Then, select **General.**


This page **must be completed** before running payroll.


![image](https://support.briohr.com/hs-fs/hubfs/image-png-Dec-09-2025-04-21-37-4503-AM.png?width=670&height=143&name=image-png-Dec-09-2025-04-21-37-4503-AM.png)


---


### Overview of Payroll General Settings


The General Settings page is divided into five (5) sections:


1. General Information
2. Registration Information
3. Bank Account
4. Calculation
5. Payslip Settings (Hide BIK and Auto Release Payslips)


---


### Section 1: General Information


Mandatory fields to complete:


- **Name** – Name of your payroll center
- **Logo** – Appears on the employee's payslip
- **Address** – Displays on EA Form, Form E, and other statutory submissions
- **Define as Main Payroll Center** – Enable if you want to combine bank and statutory files from multiple payroll centers


**Learn More: **[How to Set Up and Bulk Download Files/Reports for Main & Affiliate Payroll Centers](/knowledge/how-to-set-up-and-bulk-download-files/reports-for-main-affiliate-payroll-centers?hsLang=en)


[](https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/48301649340/original/M921FRDG47VtvcDzCog-Zf2ZHFTYY5InIA.png?1686892227)

---


### Section 2: Registration Information


Ensure you fill in all **mandatory statutory fields:**


#### 2.1 Company & Statutory Details


1. Company Registration Number (SSM)
2. Employer Tax "E" Number (LHDN)
3. Company Tax "C" Number (LHDN)
4. Employer EPF Number
5. Employer SOCSO Number

  1. If your company is part of the PSU (Program Subsidy Upah) or WSP (Wage Subsidy Programme), select the checkbox below the SOCSO number. Learn more [here](https://www.perkeso.gov.my/index.php/en/wage-subsidy-programme).


**Learn More:** [Understanding E Number, C Number, EPF Number, and SOCSO Number in Malaysia : A Complete Guide](/knowledge/understanding-e-number-c-number-epf-number-and-socso-number-in-malaysia?hsLang=en)


#### 2.2. Officer Details (Maklumat Pegawai)


These details appear on statutory documents (EA Form, Form E, CP8D)


- **Phone Number -**The company or officer’s contact number.
- **Officer Name (Nama Pegawai):** The name of your payroll officer. If not added, this **won't appear** in certain payroll documents such as the EA Form, Form E, and CP8D Form.
- **Officer IC or Passport Number**
- **Officer Position**
- **Officer Email Address**


**Learn More:** [How to Update Officer Details (Maklumat Pegawai) in EA Form and E Form in Malaysia Payroll](/knowledge/how-to-update-officer-details-maklumat-pegawai-in-ea-form-and-e-form-in-malaysia-payroll?hsLang=en)


#### 2.3. Organizations (Statutory Body Selection)


**Choose the statutory bodies your company contributes to:**


- EPF
- SOCSO
- EIS
- HRDF
- PRS (Private Retirement Scheme)
- Zakat


**IMPORTANT:**


If the statutory bodies are not selected, these statutory deductions will NOT appear during payroll runs.


![image](https://support.briohr.com/hs-fs/hubfs/image-png-Dec-09-2025-04-23-32-5990-AM.png?width=670&height=240&name=image-png-Dec-09-2025-04-23-32-5990-AM.png)

[](https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/48301649191/original/dzbe98psvgDzzS7xG7hYQcX8xkphrl0zRA.png?1686892050)
---


### Section 3: Bank Account


Make sure the following required fields are completed:


1. **Bank Account - **Select the correct company bank (e.g., Malayan Banking Berhad for Maybank)
2. **Account Holder Name**
3. **Account Number**


This is the account used for salary reimbursements.


[](https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/48294463810/original/dff0cnnaN3VrZ6L37g880u1Sp7nk5QTCGQ.png?1682479622)


**Note: The fields required to be filled in for Bank Account details may differ depending on the bank chosen.**


---


### Section 4: Calculation


These fields determine how salary, overtime, unpaid leave, and daily rates are calculated.


1. **Hours per Day: **Standard working hours.
2. **Basis for Prorate Salary Calculation:** Choose the formula your company uses for calculating salary proration. Refer to [this article](https://support.briohr.com/knowledge/prorated-salary-calculation-understanding-the-basics-in-briohr?hsLang=en) for more info.
3. **Basis for Overtime Calculation:** Formula for calculating overtime rate. Refer to [this article](https://support.briohr.com/knowledge/how-to-calculate-overtime-pay?hsLang=en) for more info.
4. **Basis for Unpaid Leave Calculation:** Formula for calculating unpaid leave. Refer to [this article](https://support.briohr.com/knowledge/how-to-calculate-unpaid-leave?hsLang=en) for more info.
5. **Basis for Daily Rate Calculation: **Formula for calculating rate for pay item following unit type "Day" type. Refer to [this article](/knowledge/basis-for-daily-rate-calculation-for-days-unit-type-pay-item?hsLang=en) for more info.
6. **Income Basis for Overtime Calculation: **Overtime calculations will be based on the basic salary and the items selected here **(optional, see explanation below).**
7. **Income Basis for Unpaid Leave Calculation:** Unpaid leave calculations will be based on the basic salary and the items selected here **(optional, see explanation below).**
8. **Income Basis for PRS Calculation: **PRS contribution calculations will be based on the basic salary and the items selected here **(optional, see explanation below).**
9. **HPRS percentage of income:** Enter the percentage for the employer's contribution. Refer to [this article](/knowledge/how-to-enable-private-retirement-scheme-prs-contributions-in-payroll-malaysia?hsLang=en) for more info on setting up PRS contributions.
10. **Does "Travel/Petrol Allowance (Official Duties)" contribute to EPF?: **Toggle on if this payitem is included in EPF calculations. Refer to [this article](/knowledge/payitem-guide-travel/petrol-allowance-malaysia?hsLang=en) for more info.
11. **Set default Overtime item as Fixed/Additional Remuneration:** LHDN (PCB) calculations for Overtime earnings that are paid out using default overtime payroll items will be based on this remuneration type. Refer to [this article](/knowledge/setting-default-overtime-payroll-items-as-fixed-or-additional-remuneration?hsLang=en) for more info.


**Explanation:

**If your Overtime, Unpaid Leave, and/or PRS calculations are based on **basic salary + <additional pay item>**, select the additional pay item here.


**Example: **


If you calculate Overtime, Unpaid Leave, PRS based on:


*Basic Salary + Travel/Petrol Allowance (For official duties),*


Then, select the relevant pay item *(Travel/Petrol Allowance)* in this field.


[](https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/48293344558/original/ePi9EnPywR8l0_9hrTJdkTg6mnmpW1Y1ew.png?1681867582)


---


### Section 5: Payslip Settings


This section controls how information appears on employee payslips.


![image](https://support.briohr.com/hs-fs/hubfs/image-png-Aug-19-2026-01-18-50-7058-AM.png?width=952&height=616&name=image-png-Aug-19-2026-01-18-50-7058-AM.png)





1. **Hide benefits in kind (BIK)**
2. **Auto Release Payslip**
3. **Payslip Annual Leave Summary**
4. **Payslip Medical Leave Summary**
5. **Payslip YTD Summary**
6. **Payslip Disclaimer**
7. **Pasword Protect Payslip**


####


#### 5.1. Hide Benefits in Kind (BIK)


Enable this if you want BIK items to be hidden from the payslip. However, they will still appear in payroll reports for the payroll month you have added the BIK items.


**Learn More:** [How to Hide Benefits-in-Kind on BrioHR Payslips](/knowledge/hiding-benefits-in-kind-on-briohr-payslips?hsLang=en)


#### 5.2. Auto Release Payslip


- **When enabled:
**Employees can view their payslip immediately after payroll is confirmed.
- **When disabled:
**Admins can manually schedule or release payslips.


**Learn More:** [How to Schedule Payslip Release in BrioHR (Payroll Admin Guide)](/knowledge/how-to-schedule-payslip-release-in-briohr-payroll-admin-guide?hsLang=en)


#### 5.3. Payslip Annual Leave Summary


Enable this if you want to display the Annual Leave Summary section in the employee's payslips.


![image](https://support.briohr.com/hs-fs/hubfs/image-png-Aug-19-2026-01-26-14-4768-AM.png?width=1264&height=334&name=image-png-Aug-19-2026-01-26-14-4768-AM.png)





#### 5.4. Payslip Medical Leave Summary


#### Enable this if you want to display the Medical Leave Summary section in the employee's payslips.


**
![image](https://support.briohr.com/hs-fs/hubfs/image-png-Aug-19-2026-01-25-48-3143-AM.png?width=1264&height=334&name=image-png-Aug-19-2026-01-25-48-3143-AM.png)
**


**Note: **


**These Leave Summary settings will not be visible for companies that are not subscribed to the BrioHR Leave module.

If the employee is not assigned to an Annual Leave or Medical Leave policy, this section will not appear in the employee's payslip even if the setting has been enabled. **


#### 5.5. Payslip YTD Summary


Enable this if you want to display the employee's YTD Summary section in the employee's payslips.


![image](https://support.briohr.com/hs-fs/hubfs/image-png-Aug-19-2026-01-48-17-8776-AM.png?width=1273&height=335&name=image-png-Aug-19-2026-01-48-17-8776-AM.png)


#### 5.6. Payslip Disclaimer


Enable this if you want to show a disclaimer message at the bottom of the employee's generated payslips. This message can be customized. However, there is a 200-character limit for the message.


![image](https://support.briohr.com/hs-fs/hubfs/image-png-Mar-27-2026-07-32-04-5257-AM.png?width=670&height=128&name=image-png-Mar-27-2026-07-32-04-5257-AM.png)


#### 5.7. Password Protect Payslip


Enable this if you want your employees to enter in a password when they access their payslip sent via email.


**Learn More:** [Enhanced Payslip Security with Password-Protected Payslips](/knowledge/enhance-payslip-security-with-password-protected-payslips?hsLang=en)


Once all fields are completed, click **Save **to apply the changes.





**IMPORTANT: **


**Please click 'Save' at the bottom of the page after ANY changes. Else, your changes will not reflect in the system. **


---


### Frequently Asked Questions (FAQs)


**1. What happens if I skip a mandatory field?
**Missing information may prevent payroll from being processed or statutory files from being generated.**

2. Can I update payroll settings after running payroll?
**Yes, but it is recommended to update settings before starting the payroll cycle.**

3. Why are my statutory calculations not appearing during payroll?
**You may have forgotten to select the statutory organizations (EPF, SOCSO, EIS, HRDC, PRS, Zakat)**

4. Why is the wrong bank file generated?
**Ensure you selected the correct bank that your company uses for salary payment.
**
5. Can I hide only certain BIK items from payslips?
**No, the Hide Benefits-in-kind hides all BIK items from the payslip.





**Ownership**


Last updated by: Arveena

